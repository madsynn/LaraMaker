            ...
            @if(\App\Models\Language::count()> 1)
                ...
                <span class="input-group-addon" data-toggle="modal" href="#translations-modal-name"><i
                    ...
                    <input class="form-control" type="text" name="lang_name[{{option('language')}}]"
                           ...
                           value="{{$values?$page->translations()->where('column', 'name')->where('lang_id',option('language'))->first()->value:\Request::old('lang_name')[option('language')]}}"/>
                </div>
            @else
                <input class="form-control" type="text" name="lang_name[{{option('language')}}]"
                ...
                <div class="panel-body">
                    <select name="page_role" class="form-control">
                        ...
                        @foreach(\App\Models\PageRole::get() as $role)
                            <option value="{{$role->id}}" {!! $values?($role->page_id == $page->id?"selected":""):($role->id==\Request::old('page_role')?"selected":"") !!}>{!! $role->name !!}</option>
                        @endforeach
                    </select>
                </div>
            </div>
...
@section('modals')
    {!! translations_modal('translations-modal-name','name','lang_name',$values?$page:"",$values) !!}
